...
tenants/london/                  # Home tenant (internal; template for new tenants)
...
cp tenants/london/{__init__.py,inventory.py,vars.py,run.py} tenants/my-infrastructure/
```
...
- **Location groups** (`paris`, `london`, `newyork`, `prod`, `staging`) — inherit privilege config from OS groups

Groups allow targeting with `--limit group_name`. Example home tenant groups:
```python
# OS groups (privilege escalation)
openbsd=(["controller.paris"], {"_doas": True}),
freebsd=(["virt.london", "virt-01.newyork", ...], {"_sudo": True}),
# Location groups (no extra config — inherit from OS groups)
work=(["controller.paris"], {}),
newyork=(["virt-01.newyork", "virt-02.newyork", "virt-03.newyork"], {}),
```

Usage:
```bash
./tenants/my-infra/run.py --dry --limit newyork       # all newyork hosts
./tenants/my-infra/run.py --dry --limit work       # paris location only
...
4. Test with home tenant: `./tenants/london/run.py --dry`
