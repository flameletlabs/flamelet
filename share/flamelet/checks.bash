...
_isGit_() {
    # DESC:
    #					Validates that input is a valid git URL
    # ARGS:
    #					$1 (required): Input to check
    # OUTS:
    #					0 - Is valid git URL
    #					1 - Is not valid git URL
    # USAGE:
    #					_isGit_ "git@github.com:foo/bar.git"
    ...
    local _emailRegex
    _gitRegex="^(https?|git)(:\/\/|@)([^\/:]+)[\/:]([^\/:]+)\/(.+).git$"
    [[ ${1} =~ ${_gitRegex} ]] && return 0 || return 1
}