...
PYTHON = "python3"
TENANT_RUN = "tenants/london/run.py"
...
class TestTenantDeployment:
    ...
    def test_dry_run_single_host_all_tasks(self):
        ...
        result = subprocess.run(
            [PYTHON, TENANT_RUN, "--dry", "--limit", "controller.paris"],
            capture_output=True,
            text=True,
        )
    ...
    def test_dry_run_single_host_groups_only(self):
        ...
        result = subprocess.run(
            [PYTHON, TENANT_RUN, "--dry", "--limit", "controller.paris", "--task", "groups"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 0, f"stderr: {result.stderr}"
    ...
    def test_dry_run_single_host_users_only(self):
        ...
        result = subprocess.run(
            [PYTHON, TENANT_RUN, "--dry", "--limit", "controller.paris", "--task", "users"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 0, f"stderr: {result.stderr}"
    ...
    def test_dry_run_verbose(self):
        ...
        result = subprocess.run(
            [PYTHON, TENANT_RUN, "--dry", "--verbose", "--limit", "controller.paris"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 0, f"stderr: {result.stderr}"
    ...
    def test_dry_run_freebsd_host(self):
        ...
        result = subprocess.run(
            [PYTHON, TENANT_RUN, "--dry", "--limit", "virt.london"],
            capture_output=True,
            text=True,
        )
        assert result.returncode == 0, f"stderr: {result.stderr}"
