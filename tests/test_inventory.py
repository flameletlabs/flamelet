...
from tenants.london.inventory import build_inventory  # noqa: E402
...
class TestTenantInventory:
    ...
    def test_openbsd_doas_enabled(self):
        ...
        inventory = build_inventory()
        controller = next(h for h in inventory if h.name == "controller.paris")
    ...
    def test_bootstrap_users(self):
        ...
        docker = next(h for h in inventory if h.name == "docker.london")
    ...
    def test_group_openbsd(self):
        """openbsd group should contain only controller.paris."""
        inventory = build_inventory()
        openbsd_group = list(inventory.get_group("openbsd"))
        assert len(openbsd_group) == 1
        assert openbsd_group[0].name == "controller.paris"
    ...
    def test_group_location_work(self):
        """work group should contain only controller.paris."""
        inventory = build_inventory()
        work_group = list(inventory.get_group("work"))
        assert len(work_group) == 1
        assert work_group[0].name == "controller.paris"

    def test_group_location_home(self):
        """home group should contain only virt.london."""
        inventory = build_inventory()
        home_group = list(inventory.get_group("home"))
        assert len(home_group) == 1
        assert home_group[0].name == "virt.london"

    def test_group_location_baar(self):
        """newyork group should contain 3 newyork hosts."""
        inventory = build_inventory()
        newyork_group = list(inventory.get_group("newyork"))
        assert len(newyork_group) == 3
        baar_names = {h.name for h in newyork_group}
        expected = {"virt-01.newyork", "virt-02.newyork", "virt-03.newyork"}
        assert baar_names == expected

    def test_group_location_pangea(self):
        """tokyo group should contain only nas-01.tokyo."""
        inventory = build_inventory()
        tokyo_group = list(inventory.get_group("tokyo"))
        assert len(tokyo_group) == 1
        assert tokyo_group[0].name == "nas-01.tokyo"

    def test_group_location_docker(self):
        """docker group should contain only docker.london."""
        inventory = build_inventory()
        docker_group = list(inventory.get_group("docker"))
        assert len(docker_group) == 1
        assert docker_group[0].name == "docker.london"

    def test_group_debian(self):
        """debian group should contain only docker.london."""
        inventory = build_inventory()
        debian_group = list(inventory.get_group("debian"))
        assert len(debian_group) == 1
        assert debian_group[0].name == "docker.london"
