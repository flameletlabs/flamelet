"""Repo-wide guard: no private infrastructure may be committed.

flamelet is a public repository (see the MANDATORY section at the top of
CLAUDE.md). That rule previously existed only as prose, and prose does not
fail a build. On 2026-08-06 a full history rewrite had to be run because
internal hostnames, real subnets and Tailscale node IPs had accumulated in
docstrings, docs and commit messages.

WHY EVERY CHECK CARRIES A CONTROL
---------------------------------
The same remediation was nearly declared complete twice on the strength of a
probe that could not fire:

  * ``git grep -E`` silently ignores ``\\b``. On one internal-hostname pattern
    it reported 0 hits where ``git grep -P`` found 2 and plain ``grep`` found 18.
  * BSD/macOS ``sed -E`` also silently ignores ``\\b`` — the substitution just
    does not happen, with no error.
  * A hand-written verification pattern enumerated specific hostnames instead
    of a generic ``[a-z0-9-]+\\.<tld>`` form, so one host slipped through into
    133 published blobs.

Every one of those failures looked exactly like success. So each check below
ships with a planted positive, and ``test_probe_fires_on_planted_violation``
asserts the probe detects it. A check that cannot fail is not a check.

The planted controls are assembled from fragments at runtime so this file does
not itself contain the literal strings it forbids — that keeps it inside the
scan rather than needing an exemption.
"""
...
#
# The leak that escaped the 2026-08-06 remediation was `gateway.home` — a host
# under a PRIVATE, non-ICANN TLD. A public-TLD check cannot see it, because
# `.home` is not a TLD anyone can look up. So this inverts the test: every
# trailing label must be on the list below, and anything unrecognised FAILS.
#
# Failing closed is deliberate. A new private TLD (`.baar`, `.atlantis`,
