...
```bash
mkdir -p tenants/flamelet-example
cp tenants/london/{__init__.py,inventory.py,vars.py,run.py} tenants/london/
```
...
```toml
[tenants]
home = "~/src/my-infrastructure/tenants/flamelet-example"
```

Deploy:

```bash
~/src/my-infrastructure/tenants/london/run.py --dry --limit controller.paris
~/src/my-infrastructure/tenants/london/run.py --task users
```
