...
def add_dnsmasq_ops(state, hosts, config, target_hosts=None, task="all"):
    ...
    Args:
        ...
        config: dict mapping hostname → dnsmasq config
            {
                "virt-01.newyork": {
                    "listen": ["10.20.0.1", "127.0.0.1"],
                    "servers": ["1.1.1.1", "8.8.8.8"],
                    "address": ["/postgresql.newyork.london/10.20.0.100", ...],
                    "server": ["/london./10.10.0.2", "/tokyo./10.30.0.50"],
...
def _add_dnsmasq_freebsd(state, host, config):
    ...
    add_op(
        ...
        commands=[
            ...
            "grep -q 'nameserver 192.168' /etc/resolv.conf || echo 'nameserver 10.10.0.1' >> /etc/resolv.conf",
...
def _generate_dnsmasq_conf(config, os_defaults):
    ...
    if dhcp_config:
        ...
        for subnet in dhcp_subnets:
            # Format: dhcp-range=10.20.0.128,10.20.0.200,12h
