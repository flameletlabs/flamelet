...
# Markers written by the tools that commonly own /etc/resolv.conf. If any of them
# is present, that file is not ours to rewrite -- see _resolver_commands().
_RESOLV_CONF_MANAGED_MARKERS = (
...
def _resolver_commands(resolv_conf="/etc/resolv.conf"):
    """Put the local dnsmasq first in resolv.conf, without destroying the file.

    This replaced a sequence that began with

        echo 'nameserver 127.0.0.1' > /etc/resolv.conf

    which was wrong in three separate ways.

    1. It TRUNCATED the file. Any `search` domains and every other nameserver
       were discarded, so short-name lookups and any resolver reachable only via
       a second entry stopped working the moment the operation ran.
    2. On a host where another tool owns resolv.conf -- NetBird, resolvconf,
       NetworkManager, systemd-resolved -- truncating it starts a fight the
       operation cannot win: the owner rewrites the file back, so the change
       either flaps or silently reverts. A VPN mesh is the painful case, because
       its resolver is usually the ONLY way to resolve the mesh's own names.
    3. The guard on the following line was dead code. It grepped resolv.conf for
       an existing nameserver *after* the redirect above had already emptied the
       file, so the pattern could never match and the fallback address was
       appended unconditionally. That address was also a hardcoded literal from
       one deployment, which does not belong in framework logic at all.

    So: skip hosts whose resolv.conf is externally managed, and otherwise PREPEND
    rather than overwrite, keeping every existing line. Prepending matters -- a
    resolver is tried in order, so appending would leave dnsmasq consulted only
    after the existing entries had already answered.

    Still built on server.shell, so it continues to report as a change under
    --dry even when the host already complies; making that report honest is a
    separate concern from making the command safe.
    """
    marker_re = "|".join(_RESOLV_CONF_MANAGED_MARKERS)
    return [
        # Written as one shell statement: each command in a server.shell list is
        # run separately, so a guard cannot protect a command that follows it --
        # which is exactly how the dead-code bug above came about.
        "if head -5 {f} 2>/dev/null | grep -qiE '{m}'; then "
        "  echo 'resolv.conf is managed externally, leaving it alone'; "
        "elif grep -qE '^[[:space:]]*nameserver[[:space:]]+127\\.0\\.0\\.1' {f} 2>/dev/null; then "
        "  echo 'local resolver already present'; "
        "else "
        "  {{ echo 'nameserver 127.0.0.1'; cat {f} 2>/dev/null; }} > {f}.flamelet-new "
        "  && cat {f}.flamelet-new > {f} && rm -f {f}.flamelet-new; "
        "fi".format(f=resolv_conf, m=marker_re),
    ]
