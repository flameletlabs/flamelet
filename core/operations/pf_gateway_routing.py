...
- Each location has a local network (e.g., home: 10.10.0.0/24)
- Each location has a Tailscale-enabled hypervisor (virt.london)
- Hypervisor advertises local subnet via Tailscale
- Hypervisor acts as gateway for local clients to reach remote subnets

Traffic Flow:
  Client (10.10.0.x)
    → virt.london gateway (10.10.0.2)
    → pf NAT translates source to virt.london's Tailscale IP
    → Tailscale mesh routes to remote gateway
    → Remote gateway processes and replies
    → Reply comes back to virt.london's Tailscale IP
...
PF_GATEWAY_ROUTING = {
    "virt.london": {
        ...
        "local_subnet": "10.10.0.0/24",  # Local network to protect
        "local_ip": "10.10.0.2",         # Gateway IP on local network
        "vpn_interface": "tailscale0",       # VPN interface name
        "remote_subnets": [
            "10.20.0.0/24",              # newyork location
            "10.30.0.0/24",              # tokyo location
            "10.40.0.0/24",               # paris location
        ],
...
# Test with tcpdump
tcpdump -i bridge10 'dst net 10.20.0.0 mask 255.255.255.0'
tcpdump -i tailscale0 'dst net 10.20.0.0 mask 255.255.255.0'
...
def generate_pf_gateway_rules(config):
    ...
    # pf rejects an empty list literal: "{  }" makes the whole ruleset fail to
    # load, so a gateway with no remote subnets would take the firewall down
    # rather than simply route nothing. Every rule below that references
    # remote_list is meaningless without subnets, so those blocks are omitted
    # instead — the result is a valid ruleset that just does no VPN routing.
    # An "endpoint" role legitimately has nothing to route.
    ...
    if remote_subnets:
        ...
        vpn_nat_rules = omitted
        vpn_out_rules = omitted
        vpn_in_rules = omitted
