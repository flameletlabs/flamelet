...
def add_unbound_ops(state, hosts, config, target_hosts=None, task="all"):
    ...
    Args:
        ...
        config: dict mapping hostname → Unbound config
            {
                "virt.london": {
                    "listen_on": ["127.0.0.1", "10.10.0.2"],
                    "access_control": ["127.0.0.0/8 allow", "10.10.0.0/24 allow"],
                    "local_data": [
                        {"name": "docker.london.", "type": "A", "value": "10.10.0.53"}
                    ],
                    "forward_zones": [
                        {"name": ".", "addrs": ["10.10.0.1", "1.1.1.1"]}
                    ]
                }
            }
